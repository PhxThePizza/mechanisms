$(document).ready(() => {
//comments "made" by ai, code is made by hand.
// Generates a random integer between Low and High, inclusive
function randInt(Low, High) {
    return Math.floor(Math.random() * (High - Low + 1)) + Low;
}
// Mods for the game can be created by editing this object.
config = {
"version":"1.0.0",
"cheats":false,
"forestLootpool": {
...
// Generate food eating buttons
function generateFoodButtons() {
    for (const recipeName in config.recipes) {
        const button = document.createElement('button');
        button.textContent = `Craft ${formatRecipeName(recipeName)}`;
        button.id = `craft-${recipeName}`;
...
function updateInventory() {
    ...
    console.log(game.tools.miningTool);
